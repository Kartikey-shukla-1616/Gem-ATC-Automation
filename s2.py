...
def is_garbage(line):
    s = line.strip()
    # Remove page number like 4/7
    if re.fullmatch(r"\d+/\d+", s):
        return True
    # Remove lines that are only (cid:xx)... and 'n's
    if re.fullmatch(r"((\(cid\:[^\)]+\))|n)+", s):
        return True
    # Remove lines with many (cid:xx) blocks only
    if s.count("(cid:") >= 2 and all(x in " n()" or x == ":" or x.isdigit() for x in s.replace("(cid:", "").replace(")", "")):
        return True
    # Remove lines that are only 'n'/'(cid:...)'
    if len([x for x in s if x not in "n ()0123456789:cid"])==0 and len(s) > 0:
        return True
    # Remove short or almost-non-letter lines
    if len(re.findall(r'[a-zA-Z0-9\u0900-\u097F]', s)) < 2:
        return True
    # Remove if all segments are (cid:...) or just n's
    segs = s.split()
    if segs and all(seg.startswith("(cid:") or set(seg) <= {"n"} for seg in segs):
        return True
    # Remove chain of syllable doubling/junk as in your last problematic example
    # E.g. ([^\w\s()]{0,3}[अ-ह][अ-ह][ा-ै]*) repeated, with lots of (cid:xx) as well
    ...
    if (len(syllables) >= 6 and
        ...
        return True
    # Remove duplicated/garbage Devanagari bigrams/trigrams
...
class ResizeImageWindow(tk.Toplevel):
    def __init__(self, parent, img_obj, callback=None, title="Resize Image", bulk_option=False):
        ...
        self.w_scale = tk.Scale(sliders, from_=20, to=self.img_width*2, orient='horizontal', label='Width', bg="#22222A", fg="white", highlightthickness=0)
        self.w_scale.set(self.img_width)
        self.w_scale.pack(side='left', expand=True, fill='x', padx=10)
        self.h_scale = tk.Scale(sliders, from_=20, to=self.img_height*2, orient='horizontal', label='Height', bg="#22222A", fg="white", highlightthickness=0)
        ...
        tk.Button(btn_frame, text="Save", command=self.save_and_close, bg="#2573eb", fg="white").pack(side='left', padx=10)
        tk.Button(btn_frame, text="Cancel", command=self.destroy, bg="#ef4455", fg="white").pack(side='left', padx=10)
...
class GEMATCGenerator(tk.Tk):
    def __init__(self):
        ...
        self.theme_colors = [
            "#006bce", "#c0342c", "#107d4e","#d4790c","#7646d0","#148B90","#f97316","#232335"
        ]
        ...
        but_props = dict(bg="#006bce", fg="#fff", font=("Arial", 10, "bold"), padx=14, pady=4, bd=0, relief='ridge')
        ...
        tk.Button(top_frame, text="Change Theme Color", command=self.change_theme_color, bg="#7646d0", fg="#fff", font=("Arial", 10, "bold"), padx=14, pady=4).pack(side='left', padx=5)
        tk.Button(top_frame, text="Clear All", command=self.clear_all, bg="#c0342c", fg="#fff", font=("Arial", 10, "bold"), padx=14, pady=4).pack(side='left', padx=5)
        ...
        tk.Label(toolbar, text="Font Size: ", bg="#23242e", fg="#fff", font=("Arial", 10)).pack(side='left', padx=5)
        ...
        tk.Button(toolbar, text="docs pdf paster", command=self.docs_pdf_paster, bg="#f97316", fg="#fff", font=("Arial", 10, "bold")).pack(side='left', padx=7)
        tk.Button(toolbar, text="pdf paster resize", command=self.pdf_paster_resize, bg="#107d4e", fg="#fff", font=("Arial", 10, "bold")).pack(side='left', padx=3)

        self.text = ScrolledText(self, wrap=tk.WORD, font=(self.font_name, 12), undo=True, autoseparators=True, maxundo=-1, bg="#222234", fg="#e0e0e0",
                                    insertbackground="#66cccc", selectbackground="#003366", padx=12, pady=12)
        ...
        self.status = tk.Label(self, textvariable=self.status_text, bd=1, relief="sunken", anchor="w",
            bg="#384150", fg="white", font=("Arial", 10))
        self.status.pack(side="bottom", fill="x", pady=0)
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for page in pdf.pages:
                    ...
                    if extracted:
                        ...
                        for l in lines:
                            l = l.lstrip()
                            if l.startswith("/Buyer Added"):
                                l = l[1:].lstrip()
                            lines_clean.append(l)
                        text_to_add = "\n".join([l for l in lines_clean if l.strip()])
                        if text_to_add:
                            self.text.insert(tk.INSERT, text_to_add + "\n")
                    img_page = page.to_image(resolution=180).original.convert("RGB")
                    ...
                    self.text.image_create(tk.INSERT, image=img_tk)
                    self.tracked_images.append(("img", None, self.text.index(tk.INSERT), img_page, None, "center"))
    ...
    def pdf_paster_resize(self):
        ...
        if not imgs:
            ...
            return
        choices = [f"Image {i+1}" for i in imgs] + ["All"]
        if len(imgs) > 1:
            choice = simpledialog.askstring(
                "Resize Images",
                "Enter image number (e.g. 1), or type 'All' to resize all images:\n" + '\n'.join(choices)
            )
            if not choice: return
            choice = choice.strip().lower()
            if choice == "all":
                first_img = self.tracked_images[imgs[0]][3]
                ResizeImageWindow(self, first_img, self._resize_all_images, title="Resize All Images", bulk_option=True).focus()
            else:
                try:
                    selected_idx = int(choice.split()[-1]) - 1
                    if selected_idx in imgs:
                        self._launch_resize(selected_idx)
                    else:
                        messagebox.showerror("Error", f"Image {selected_idx+1} not found.")
                except Exception:
                    messagebox.showerror("Error", "Invalid selection!")
        else:
            self._launch_resize(imgs[0])
    ...
    def resize_doc(self):
        ...
        if len(imgs) > 1:
            choice = simpledialog.askstring(
                "Resize Docs",
                "Enter image number (e.g. 1), or type 'All' to resize all:\n" + '\n'.join(choices)
            )
            if not choice: return
            choice = choice.strip().lower()
            if choice == "all":
                first_img = self.tracked_images[imgs[0]][3]
                ResizeImageWindow(self, first_img, self._resize_all_images, title="Resize All Document Images", bulk_option=True).focus()
            else:
                try:
                    selected_idx = int(choice.split()[-1]) - 1
                    if selected_idx in imgs:
    ...
    def _resize_all_images(self, width, height):
        self.text.delete("1.0", "end")
        for i, item in enumerate(self.tracked_images):
            if item[0] == "img" and item[3] is not None:
                img_obj = item[3]
                resized_img = img_obj.resize((width, height), Image.LANCZOS)
                img_tk = ImageTk.PhotoImage(resized_img)
                self.tk_images.append(img_tk)
                slot = self.text.image_create("end", image=img_tk)
                self.tracked_images[i] = ("img", item[1], self.text.index("insert"), resized_img, slot, item[5] if len(item) > 5 else "center")
                self.text.insert("end", "\n")
        self.status_text.set(f"All images resized and originals deleted.")

    def _launch_resize(self, index):
        item = self.tracked_images[index]
        path, pos, old_slot, img_obj, align_tag = item[1], item[2], item[4], item[3], item[5] if len(item)>5 else "center"
        ...
        def on_resize(resized_img):
            try:
                if old_slot is not None:
                    try: self.text.image_delete(old_slot)
                    except Exception: pass
                img_tk = ImageTk.PhotoImage(resized_img)
                self.tk_images.append(img_tk)
                self.text.mark_set("insert", pos)
                slot = self.text.image_create("insert", image=img_tk)
                self.tracked_images[index] = ("img", path, pos, resized_img, slot, align_tag)
                self.text.insert("insert", "\n")
                self.status_text.set("Image resized!")
            except Exception as e:
        ...
        if img_obj is not None:
            ...
            messagebox.showerror("Error", "Image not found for resizing.")
    ...
    def track_cursor(self, event):
        try: self.last_cursor_pos = self.text.index(f"@{event.x},{event.y}")
        except: pass
    ...
    def change_theme_color(self):
        ...
        for widget in self.winfo_children():
            try: widget.configure(bg=new_color)
            except: pass
    ...
    def upload_image_file(self, place):
        filepath = filedialog.askopenfilename(title=f"Select {place} (JPG/PNG)", filetypes=[("Image Files", "*.jpg *.jpeg *.png")])
                ...
                for page in pdf.pages:
                    ...
                    for line in lines:
                        ...
                        if capture and end_marker in line:
                            capture = False
                            break
                        if capture: tc_text.append(line)
                self.text.delete("1.0", "end")
                for line in tc_text:
                    line = line.lstrip()
    ...
    def prompt_add_docs(self):
        self.doc_list = filedialog.askopenfilenames(title="Select Additional Documents (JPG/PNG)", filetypes=[("Image Files", "*.jpg *.jpeg *.png")])
        if self.doc_list:
    ...
    def insert_docs(self):
        if not self.doc_list:
            messagebox.showinfo("Info", "No documents selected."); return
        num_inserted = 0
        ...
        for doc in self.doc_list:
            ...
            if ext in (".jpg", ".jpeg", ".png"):
                try:
                    img = Image.open(doc)
                    if img.width > 600:
                        img = img.resize((600, int(600 / img.width * img.height)), Image.LANCZOS)
                    img_tk = ImageTk.PhotoImage(img)
                    self.tk_images.append(img_tk)
                    self.text.mark_set("insert", pos)
                    slot = self.text.image_create("insert", image=img_tk)
                    self.tracked_images.append(("img", doc, pos, img, slot, "center"))
                    self.text.insert("insert", "\n")
    ...
    def export_to_pdf(self):
        output_path = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=[("PDF Files", "*.pdf")], title="Save ATC as PDF")
        if not output_path: return
        Thread(target=self._pdf_preview_render, args=(output_path,)).start()
    ...
    def _pdf_preview_render(self, output_file="preview_temp.pdf"):
        ...
        heading_style = lambda sz: ParagraphStyle(name=f'TCHeading{sz}', parent=styles['Heading2'],
                                                  alignment=TA_CENTER, fontName=self.bold_font_name,
                                                  fontSize=sz, spaceAfter=12)
        normal_style = lambda sz: ParagraphStyle(name=f'TCLine{sz}', parent=styles['Normal'],
                                                 fontName=self.font_name, fontSize=sz, spaceAfter=5, alignment=TA_LEFT)
        ...
        for item in self.tracked_images:
            if item[0] == "img" and item[3] is not None:
                img, align_tag = item[3], item[5] if len(item)>5 else "center"
        ...
        frame = Frame(frame_margin_left, frame_margin_bottom, frame_width, frame_height, id='bodyframe')
    ...
    def on_page(self, canvas, doc):
        ...
        if self.header_img_resized is not None:
            ...
            canvas.drawImage(ImageReader(self.header_img_resized), x_pos, y_pos, width=scaled_width, height=scaled_height, preserveAspectRatio=True)
        if self.footer_img_resized is not None:
            ...
            x_pos = 40
            y_pos = 15
            canvas.drawImage(ImageReader(self.footer_img_resized), x_pos, y_pos, width=scaled_width, height=scaled_height, preserveAspectRatio=True)
